...
constexpr float k_climbDownwardSpeed = 180.0f;      ///< Local-down speed for intentional downward climbing (u/s).
constexpr float k_climbSidewaysAccel = 1000.0f;     ///< Wall-tangent acceleration from side input while climbing.
constexpr float k_climbSidewaysMaxSpeed = 260.0f;   ///< Max preserved wall-tangent climb speed (u/s).
constexpr float k_climbSidewaysSlipScale = 0.35f;   ///< Slip scale while actively shimmying sideways.
constexpr float k_climbNonUpDetachTime = 1.0f;      ///< Grace time before slipping/no-up climb detaches (s).
