...
bool ParticleRenderer::init(SDL_GPUDevice* dev, SDL_GPUTextureFormat colorFmt, SDL_GPUShaderFormat shaderFmt)
{
    ...
    arcBuf_.init(dev, sizeof(ArcVertex) * 8192, GpuParticleBuffer::Mode::Vertex); // railgun + energy arcs share this
...
void ParticleRenderer::drawAll(SDL_GPURenderPass* pass, SDL_GPUCommandBuffer* cmd, float screenW, float screenH)
{
    ...
    if (sdfHudPipeline_ && sdfHudBuf_.liveCount() > 0 && sdfAtlasTex_ && sdfAtlasSamp_) {
        ...
        // Vulkan NDC: Y points downward (−1 = top, +1 = bottom).
        // ortho(left, right, bottom, top): bottom=0, top=sh maps
        // pixel y=0 → NDC −1 (top) and y=sh → NDC +1 (bottom),
        // giving standard screen coordinates with (0,0) at top-left.
        ParticleUniforms hpu{};
        hpu.view = glm::mat4(1.0f);
        hpu.proj = glm::ortho(0.f, sw, 0.f, sh, -1.f, 1.f);
        hpu.camPos = {};
        hpu._p0 = 0;
