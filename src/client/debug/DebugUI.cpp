...
void DebugUI::buildUI(const Registry& registry, const int tickCount)
{
    ...
    ImGui::SetNextWindowSize({460.0f, 540.0f}, ImGuiCond_FirstUseEver);
    ...
    // Count and iterate entities via the entity storage — EnTT v3.14 API.
    const auto* const k_entityStorage = registry.storage<entt::entity>();

    int entityCount = 0;
    if (k_entityStorage)
        for (auto entity : *k_entityStorage)
    ...
    for (const entt::entity entity : *k_entityStorage) {
        ...
        char entityLabel[32];
        SDL_snprintf(entityLabel, sizeof(entityLabel), "Entity #%u", static_cast<unsigned>(entt::to_integral(entity)));

        if (ImGui::CollapsingHeader(entityLabel, ImGuiTreeNodeFlags_DefaultOpen)) {
            // -- Vec3 components table ---------------------------------------
            ...
            if (ImGui::BeginTable("##vec3", 4, k_tableFlags)) {
                ...
                // Lambda to emit one labelled vec3 row.
            ...
            if (showInputSnapshot && registry.all_of<InputSnapshot>(entity)) {
                ...
                ImGui::Text("  fwd:%-3s  back:%-3s  left:%-3s  right:%-3s"
                            ...
                            c.jump ? "Y" : "N",
                            c.crouch ? "Y" : "N");
                ImGui::Text("  yaw: %.4f   pitch: %.4f", static_cast<double>(c.yaw), static_cast<double>(c.pitch));
            }
        }
