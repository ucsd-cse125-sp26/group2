...
bool AssetLoader::loadModelsList()
{
    ...
    bool res = loadModel(id, modelFileName, texFileNames);
                            ...
                            const std::vector<std::string>& texFileNames)
{
    ...
    if (asimpSceneStructure->mNumMeshes == 0 || asimpSceneStructure->mMeshes == nullptr) {
        std::cout << "scene not found" << std::endl;
        return false;
    }

    std::cout << " if (asimpSceneStructure->mMeshes[0] == nullptr) return false; " << std::endl;
    if (asimpSceneStructure->mMeshes[0] == nullptr)
        return false;
