...
SDL_AppResult Game::iterate()
{
    ...
    // Build weapon viewmodel
    {
        ...
        if (currentWeaponRenderable && weaponVmLoaded_[vmType] && vm.visible) {
            ...
            // Crouch viewmodel pose is temporarily disabled. Apex's crouch is an
            // ADDITIVE clip, and the io_scene_cast importer drops its per-bone delta
            // motion — the clip arrives as the rest pose plus a constant 90deg
            // jx_c_delta (root mover) artifact, which rolled the whole viewmodel about
            // its root. The runtime additive support (setAdditiveLayer + ozz
            // BlendingJob) is built and correct; it just needs clean delta clip data,
            // which this import path does not yet provide. Clear any additive layer so
            // the gun holds its normal pose.
            vmGun.clearAdditiveLayer();
            if (hasArms)
                vmArms.clearAdditiveLayer();
